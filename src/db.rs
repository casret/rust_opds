...
impl DB {
    ...
    pub fn store_comic(&mut self, info: &ComicInfo, comic_info: &str) -> Result<(), Error> {
        ...
        let issue_id = stmt.insert(&[&info.filepath, &info.imported_at, &info.read_at, &info.comicvine_id, &info.series, &info.issue_number,
        ...
        stmt = self.conn.prepare_cached("insert into issue_fts(issue_id, comicinfo) values (?1, ?2)")?;
